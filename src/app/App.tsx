...
export default function App() {
  ...
  const connect = async (rawUrl: string, includeMock: boolean) => { setBusy(true); setError(''); try { const url = normalizeEndpoint(rawUrl); const client = new ApiClient(url); await client.health(); const setup=await client.setup(includeMock); if(setup.initialPin){sessionStorage.setItem('krunote.initialPin',setup.initialPin);setInitialPin(setup.initialPin)} await client.connectionTest(); localStorage.setItem('krunote.endpoint',url); setEndpoint(url); setPhase('unlock') } catch (cause) { setError(cause instanceof Error ? cause.message : 'เชื่อมต่อไม่สำเร็จ') } finally { setBusy(false) } }
  ...
  const changePin = async (current: string,next: string) => { if (!api || !sessionRef.current) throw new Error('กรุณาปลดล็อกอีกครั้ง'); setBusy(true); setError(''); try { const session = await api.changePin(sessionRef.current.token,current,next); const key = await createCryptoSession(next); keyRef.current = key; sessionRef.current = session; sessionStorage.removeItem('krunote.initialPin'); setInitialPin(''); if (data) await saveBootstrap(key,data); setPhase('app') } catch (cause) { const message=cause instanceof Error ? cause.message : 'เปลี่ยน PIN ไม่สำเร็จ'; setError(message); throw cause } finally { setBusy(false) } }
