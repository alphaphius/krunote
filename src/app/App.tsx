...
export default function App() {
  ...
  const changePin = async (current: string,next: string) => { if (!api || !sessionRef.current) return; setBusy(true); setError(''); try { const session = await api.changePin(sessionRef.current.token,current,next); const key = await createCryptoSession(next); keyRef.current = key; sessionRef.current = session; if (data) await saveBootstrap(key,data); setPhase('app') } catch (cause) { setError(cause instanceof Error ? cause.message : 'เปลี่ยน PIN ไม่สำเร็จ') } finally { setBusy(false) } }
  ...
  if (phase === 'unlock') return <UnlockPage locale={locale} onUnlock={unlock} onBack={() => setPhase('setup')} busy={busy} error={error} hasOffline={cached} />
