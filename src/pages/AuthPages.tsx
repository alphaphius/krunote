...
export function UnlockPage({ locale, onUnlock, onBack, busy, error, hasOffline, initialPin }: { locale: Locale; onUnlock: (pin: string) => Promise<void>; onBack?: () => void; busy: boolean; error?: string; hasOffline: boolean; initialPin?: string }) {
  const t = dictionary(locale)
  ...
  return <main className="auth-shell"><section className="auth-panel"><Brand title={t.appName} subtitle={t.tagline} /><h1>{t.unlockTitle}</h1><p className="muted">{hasOffline ? t.offlineUnlock : initialPin ? <>{t.initialPinHint} <strong className="initial-pin">{initialPin}</strong></> : t.initialPinHint}</p><PinPad value={pin} onChange={setPin} maxLength={12} />{error && <p className="error-copy" role="alert">{error}</p>}<div className="stack auth-actions"><Button onClick={() => void onUnlock(pin)} disabled={busy || pin.length < 4}>{busy ? t.unlocking : t.unlock}</Button>{onBack&&<Button variant="ghost" onClick={onBack}>{t.cancel}</Button>}</div></section></main>
}
...
export function ChangePinPage({ locale, onChangePin, busy, error }: { locale: Locale; onChangePin: (current: string, next: string) => Promise<void>; busy: boolean; error?: string }) {
  ...
  return <main className="auth-shell"><section className="auth-panel"><Brand title={t.appName} subtitle={t.tagline} /><h1>{t.changePinTitle}</h1><form className="stack" onSubmit={(event) => { event.preventDefault(); void onChangePin(current, next).catch(() => undefined) }}><Field label={t.currentPin}><Input type="password" inputMode="numeric" value={current} onChange={(event) => setCurrent(event.target.value.replace(/\D/g, '').slice(0, 12))} required /></Field><Field label={t.newPin}><Input type="password" inputMode="numeric" value={next} onChange={(event) => setNext(event.target.value.replace(/\D/g, '').slice(0, 12))} required /></Field><Field label={t.confirmPin} error={localError || error}><Input type="password" inputMode="numeric" value={confirm} onChange={(event) => setConfirm(event.target.value.replace(/\D/g, '').slice(0, 12))} required /></Field><Button type="submit" disabled={busy || Boolean(localError) || next.length < 6}>{t.changePin}</Button></form></section></main>
}
